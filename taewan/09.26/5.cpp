...
vector<int> solution(int n) {
  ...
  if (n == 1 || n == 2) return except(n);
  int cnt = 0;
  int result = n * (n + 1) / 2;
  for (int i = 0; i < n; i++) vec[i][0] = ++cnt;
  for (int i = 1; i < n; i++) vec[n - 1][i] = ++cnt;
  for (int i = n - 2; i != 0; i--) vec[i][i] = ++cnt;
  int y = 1, x = 1;
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      if (vec[i][j] != 0) answer.push_back(vec[i][j]);
      else break;
    }
  }
  return answer;
